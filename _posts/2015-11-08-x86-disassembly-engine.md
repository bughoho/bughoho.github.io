...
##4. Capstone  
　　[`capstone`](https://github.com/aquynh/capstone)应该算是所有反汇编引擎中集大成者了，对于它我要多费点口水，因为我对他是又爱又恨。
...
另外capstone的`next`分支（`master`分支没有这个接口）中也支持BeaEngine那种解码时分析指令使用和影响到的寄存器这种炫酷的特技，有这样的基础库存在真的可以偷不少的懒。  
...
原创文章，来源请注明出处:[bughoho.me](/disassembly/2015/11/08/x86-disassembly-engine.html)